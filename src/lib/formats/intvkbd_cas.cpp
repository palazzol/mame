...
static cassette_image::error intvkbd_ikc_identify ( cassette_image *cass, cassette_image::Options* opts)
{
	...
	if (opts->bits_per_sample != 16)            // this is the only one supported for now
		return cassette_image::error::INVALID_IMAGE;
	if (opts->channels != 4)                    // required
		return cassette_image::error::INVALID_IMAGE;
	if (opts->sample_frequency != 48000)        // this is the only one supported for now
		return cassette_image::error::INVALID_IMAGE;
	return cassette_image::error::SUCCESS;
}
...
CASSETTE_FORMATLIST_START(intvkbd_cassette_formats)
	CASSETTE_FORMAT(intvkbd_cassette_format)
