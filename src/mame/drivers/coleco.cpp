...
void bit90_state::init()
{
	...
	if(m_ram_extra->size() == 32768)
		return;
	else if(m_ram_extra->size() == 16384)
		pgm->unmap_readwrite(0xc000, 0xffff);
	else if(m_ram_extra->size() == 1024)
...
void bit90_state::bit90_map(address_map &map)
{
	...
	map(0x8000, 0xffff).ram().share("ram_extra");
}
...
void bit90_state::bit90(machine_config &config)
{
	...
	/* internal ram */
	RAM(config, m_ram_extra).set_default_size("1K").set_extra_options("16K,32K");
